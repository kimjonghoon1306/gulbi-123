...
export default function WholesalePage() {
  ...
  const [form, setForm] = useState({ company_name: '', contact: '', address: '', note: '', payment_method: '가상계좌', status: '접수', courier_code: '', tracking_number: '' })
  ...
  const resetForm = () => {
    setForm({ company_name: '', contact: '', address: '', note: '', payment_method: '가상계좌', status: '접수', courier_code: '', tracking_number: '' })
  ...
  const saveTracking = async () => {
    ...
    await supabase.from('wholesale_orders').update(patch).eq('id', viewOrder.id)
  ...
  const downloadExcel = () => {
    const headers = ['주문번호', '업체명', '연락처', '배송지', '결제방법', '상태', '금액', '주문일시']
    ...
    a.href = url
    a.download = '도매주문_' + new Date().toLocaleDateString('ko-KR').replace(/\./g, '').replace(/ /g, '') + '.xls'
    a.click()
  ...
  const refundOrder = async (o: Order) => {
    ...
    await supabase.from('wholesale_orders').update({ status: '환불', updated_at: new Date().toISOString() }).eq('id', o.id)
    ...
    <div className="animate-fadeIn overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-5">
        <div className="overflow-x-auto pb-1 min-w-0 flex-1"><div className="flex gap-2 flex-nowrap">
          {['전체', ...STATUS_LIST].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)}
              ...
              style={filterStatus === s ? { background: 'linear-gradient(135deg,#7c3aed,#6366f1)' } : {}}>
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="w-full border border-slate-200 dark:border-gray-600 rounded-xl px-4 py-2 text-sm bg-white dark:bg-gray-800 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-400" />
      </div>
              ...
              <div className="flex gap-1.5 min-w-max">
                {STATUS_LIST.map(s => (
                  <button key={s} onClick={() => updateStatus(o.id, s)}
                    ...
                    style={o.status === s ? { background: 'linear-gradient(135deg,#7c3aed,#6366f1)' } : {}}>
      ...
      {viewOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setViewOrder(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-fadeIn" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-100 dark:border-gray-700" style={{ background: 'linear-gradient(135deg,#7c3aed,#6366f1)' }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white/70 text-xs mb-1">{viewOrder.order_number}</p>
                  <h2 className="text-xl font-bold text-white truncate">{viewOrder.company_name || '거래처'}</h2>
                </div>
                ...
                <div className="space-y-2">
                  {viewItems.map((item, i) => (
                    <div key={i} className="flex justify-between items-center bg-slate-50 dark:bg-gray-700/50 rounded-xl px-4 py-3">
                      ...
                      <p className="text-sm font-bold text-violet-600 dark:text-violet-400">{item.total_price.toLocaleString()}원</p>
                    </div>
                  ))}
                </div>
                <div className="flex justify-between items-center mt-4 px-4 py-3 rounded-xl" style={{ background: 'linear-gradient(135deg,rgba(124,58,237,0.08),rgba(99,102,241,0.08))' }}>
                  <span className="font-bold text-slate-800 dark:text-white">합계</span>
                  <span className="text-xl font-bold text-violet-600 dark:text-violet-400">{viewOrder.total_amount.toLocaleString()}원</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2 flex-nowrap">
                  {STATUS_LIST.map(s => (
                    <button key={s} onClick={() => updateStatus(viewOrder.id, s)}
                      ...
                      style={viewOrder.status === s ? { background: 'linear-gradient(135deg,#7c3aed,#6366f1)' } : {}}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <button onClick={saveTracking}
                    ...
                    style={{ background: trackSaved ? '#16a34a' : 'linear-gradient(135deg,#7c3aed,#6366f1)' }}>
                    {trackSaved ? '✓ 송장 저장됨' : '🚚 송장 저장'}
                  </button>
                </div>
                <p className="text-[11px] text-slate-400 mt-1.5">송장 등록 시 거래처가 마이페이지에서 실시간 배송조회를 할 수 있어요.</p>
              </div>
              <div className="flex gap-2 pt-2">
                <button onClick={() => { setViewOrder(null); openEdit(viewOrder) }} className="flex-1 py-2.5 rounded-xl text-sm font-bold border-2 border-violet-200 dark:border-violet-800 text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-colors">✏️ 수정</button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fadeIn" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-gray-700">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">도매 거래처 주문</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">🏭 거래처 정보</p>
                  ...
                  {[
                    ...
                    <div key={f.key} className={f.col === 2 ? 'col-span-2' : ''}>
                      ...
                      <input type="text" placeholder={f.placeholder} value={(form as any)[f.key]}
                        ...
                        className="w-full border border-slate-200 dark:border-gray-600 rounded-xl px-4 py-3 text-sm bg-white dark:bg-gray-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-400" />
                    </div>
                  ))}
                  <div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={addItem} className="text-xs font-bold text-violet-500 hover:text-violet-600 bg-violet-50 dark:bg-violet-900/20 hover:bg-violet-100 px-3 py-1.5 rounded-lg transition-colors">+ 상품 추가</button>
                </div>
                <div className="space-y-3">
                  {items.map((item, i) => (
                    <div key={i} className="bg-slate-50 dark:bg-gray-700/50 rounded-xl p-4 space-y-3 border border-slate-100 dark:border-gray-600">
                      <div className="flex gap-3">
                        <div className="flex-1">
                          ...
                          <select value={item.product_id} onChange={e => updateItem(i, 'product_id', e.target.value)}
                            className="w-full border border-slate-200 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-400">
                            <option value="">직접 입력</option>
                            {products.map(p => <option key={p.id} value={p.id}>{p.name} ({p.wholesale_price.toLocaleString()}원/{p.unit})</option>)}
                          </select>
                        </div>
                        {items.length > 1 && (
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-sm font-bold text-violet-600 dark:text-violet-400">{item.total_price.toLocaleString()}원</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="flex justify-between items-center mt-4 px-4 py-3 rounded-xl" style={{ background: 'linear-gradient(135deg,rgba(124,58,237,0.08),rgba(99,102,241,0.08))' }}>
                  <span className="font-bold text-slate-800 dark:text-white">합계</span>
                  <span className="text-2xl font-bold text-violet-600 dark:text-violet-400">{totalAmount.toLocaleString()}원</span>
                </div>
              </div>
            </div>
