  ...
  return (
    <div className="animate-fadeIn">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">소매주문관리</h1>
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">개인 고객 소매 주문 접수 및 관리</p>
        </div>
        <button onClick={() => { resetForm(); setShowForm(true) }}
          className="bg-violet-500 hover:bg-violet-400 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 active:scale-95 shadow-md shadow-violet-500/20">
          + 주문 등록
        </button>
      </div>
        ...
        {['전체', ...STATUS_LIST].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200
              ${filterStatus === s ? 'bg-violet-500 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-gray-700 hover:bg-slate-50 dark:hover:bg-gray-700'}`}>
              ...
              {STATUS_LIST.map(s => (
                <button key={s} onClick={() => updateStatus(o.id, s)}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all duration-150
                    ${o.status === s ? 'bg-violet-500 text-white' : 'bg-slate-50 dark:bg-gray-700 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-gray-600'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-fadeIn">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  {viewItems.map((item, i) => (
                    <div key={i} className="flex justify-between items-center bg-slate-50 dark:bg-gray-700/50 rounded-xl px-4 py-3">
                      ...
                      <p className="text-sm font-bold text-violet-500">{item.total_price.toLocaleString()}원</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-slate-100 dark:border-gray-700">
                ...
                <span className="text-xl font-bold text-violet-500">{viewOrder.total_amount.toLocaleString()}원</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fadeIn">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={addItem} className="text-xs text-violet-500 hover:text-violet-600 font-medium">+ 상품 추가</button>
                </div>
                <div className="space-y-3">
                  {items.map((item, i) => (
                    <div key={i} className="bg-slate-50 dark:bg-gray-700/50 rounded-xl p-4 space-y-3">
                      <div className="flex gap-3">
                        <div className="flex-1">
                          ...
                          <select value={item.product_id} onChange={e => updateItem(i, 'product_id', e.target.value)}
                            className="w-full border border-slate-200 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-500">
                            <option value="">직접입력</option>
                            {products.map(p => <option key={p.id} value={p.id}>{p.name} ({p.retail_price.toLocaleString()}원)</option>)}
                          </select>
                        </div>
                        {items.length > 1 && (
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div>
                          ...
                          <input type="number" value={item.unit_price} onChange={e => updateItem(i, 'unit_price', Number(e.target.value))}
                            className="w-full border border-violet-200 dark:border-violet-700 rounded-lg px-3 py-2 text-sm bg-violet-50 dark:bg-violet-900/20 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-500 font-medium" />
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-sm font-bold text-violet-500">{item.total_price.toLocaleString()}원</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="flex justify-between items-center mt-4 px-1">
                  ...
                  <span className="text-xl font-bold text-violet-500">{totalAmount.toLocaleString()}원</span>
                </div>
              </div>
            </div>
            <div className="flex gap-3 p-6 border-t border-slate-100 dark:border-gray-700">
              ...
              <button onClick={saveOrder} className="flex-1 py-3 rounded-xl bg-violet-500 hover:bg-violet-400 text-white text-sm font-bold transition-colors active:scale-95">
