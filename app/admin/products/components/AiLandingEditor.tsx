    ...
    <>
      ...
      {aiTab === 'ai' && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: aiDark ? 'rgba(0,0,0,0.95)' : 'rgba(240,240,240,0.97)', backdropFilter: 'blur(10px)', display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
          ...
          {/* STEP 2: 상품 정보 + 말투 */}
          {!aiChoice && aiStep === 2 && (
            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: isMobile ? '72px 14px 28px' : '24px 20px', background: aiDark ? 'transparent' : '#f8f8f8', position: 'relative' }}>
              {aiLoading && (
