...
<section data-section="hero" style="background:${C.heroGrad};padding:56px 28px 72px;text-align:center;position:relative;overflow:hidden;">
  ...
  <div style="position:relative;z-index:2;">
    ...
    ${img ? `<div style="margin:32px auto 0;max-width:320px;aspect-ratio:1/1;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);border-radius:8px;overflow:hidden;box-shadow:0 20px 40px rgba(0,0,0,0.5);">${imgTag(img, d.productName, 'width:90%;height:90%;object-fit:contain;')}</div>` : ''}
  </div>
</section>`
}
...
<section data-section="origin" style="background:${C.ink};color:${C.cream};padding:80px 28px 0;">
  ...
  <p ${ce('origin.story')} style="font-family:${F.deco};font-size:15px;line-height:1.95;opacity:0.85;margin:0 0 56px;">${esc(d.originStory)}</p>
  <div style="margin:0 -28px 0;height:280px;background:linear-gradient(180deg, transparent 0%, rgba(0,0,0,0.6) 100%), ${C.originGrad};position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;">
    ${img ? imgTag(img, d.productName, 'max-height:180px;max-width:60%;object-fit:contain;filter:drop-shadow(0 20px 40px rgba(0,0,0,0.5));') : ''}
    <span ${ce('origin.tag')} style="position:absolute;bottom:20px;left:28px;font-family:${F.serif};font-size:11px;letter-spacing:0.3em;color:${C.cream};opacity:0.8;">${esc(d.originLocation)}</span>
  </div>
...
<section data-section="story" style="padding:80px 28px;background:${C.cream};">
  ...
  ${img ? `<div style="margin:0 0 32px;overflow:hidden;background:${C.imgBg};aspect-ratio:16/10;display:flex;align-items:center;justify-content:center;">${imgTag(img, d.productName, 'max-width:70%;max-height:80%;object-fit:contain;')}</div>` : ''}
  <div ${ce('story.body')} style="font-family:${F.deco};font-size:15px;line-height:2.05;color:${C.inkSoft};">${paragraphs}</div>
</section>`
}
...
<section data-section="recipe" style="padding:80px 28px;background:${C.cream};">
  ...
  ${img ? `<div style="margin:0 0 40px;overflow:hidden;background:${C.imgBg};aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;box-shadow:0 12px 32px rgba(0,0,0,0.25);">${imgTag(img, d.productName, 'max-width:55%;max-height:75%;object-fit:contain;')}</div>` : ''}
  <div>
...
<section data-section="storage" style="padding:80px 28px;background:${C.paper};">
  ...
  ${img ? `<div style="margin:0 0 32px;aspect-ratio:16/9;background:${C.imgBg};display:flex;align-items:center;justify-content:center;overflow:hidden;">${imgTag(img, d.productName, 'max-width:55%;max-height:75%;object-fit:contain;')}</div>` : ''}
  <div>
