...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white overflow-x-hidden w-full">
      ...
      <a
        ...
        className="fixed bottom-6 right-4 sm:bottom-8 sm:right-8 z-50 flex items-center gap-2 text-white text-sm font-bold px-4 py-3 sm:px-5 sm:py-3.5 rounded-full shadow-2xl select-none"
        style={{
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#0a0f1e]/95 backdrop-blur-xl shadow-xl shadow-black/30 border-b border-white/5' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-6">
            ...
            <Link href="/shop"
              className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105">
              🛒 <span className="hidden sm:inline">쇼핑몰</span>
            </Link>
            <Link href="/admin/dashboard"
              className="flex items-center gap-1.5 bg-sky-500 hover:bg-sky-400 text-white text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-sky-500/30">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-4">
        ...
        <div className={`relative z-10 text-center w-full max-w-4xl mx-auto transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        {[
          ...
          <div key={i} className="absolute text-3xl sm:text-4xl opacity-20 select-none pointer-events-none hidden sm:block"
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#0a0f1e]/95 backdrop-blur-xl shadow-xl shadow-black/30 border-b border-white/5' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg tracking-tight">굴비가게</span>
          </div>
          <div className="flex items-center gap-6">
            ...
            <a href="#contact" className="text-sm text-slate-400 hover:text-white transition-colors hidden md:block">문의</a>
            <Link href="/shop"
              className="flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105">
              🛒 쇼핑몰
            </Link>
            <Link href="/admin/dashboard"
              className="flex items-center gap-2 bg-sky-500 hover:bg-sky-400 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-sky-500/30">
              ⚙️ 관리자
            </Link>
          </div>
        </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className="absolute top-0 left-0 right-0 h-full bg-gradient-to-b from-sky-950/40 via-transparent to-transparent" />
          <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-sky-600/15 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-1/3 right-1/4 w-80 h-80 bg-blue-700/15 rounded-full blur-3xl animate-pulse" style={{animationDelay:'1.5s'}} />
          <div className="absolute top-1/2 right-1/3 w-64 h-64 bg-cyan-600/10 rounded-full blur-3xl animate-pulse" style={{animationDelay:'3s'}} />
          {/* 그리드 패턴 */}
          <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage: 'linear-gradient(#fff 1px, transparent 1px), linear-gradient(90deg, #fff 1px, transparent 1px)', backgroundSize: '60px 60px'}} />
        </div>
        ...
        <div className={`relative z-10 text-center max-w-4xl mx-auto px-4 sm:px-6 transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center gap-2 bg-sky-500/10 border border-sky-500/20 text-sky-400 text-xs font-semibold px-5 py-2.5 rounded-full mb-8 backdrop-blur">
            🐟 수산물 도매업체를 위한 전문 관리 솔루션
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-8xl font-black mb-6 leading-[1.1] tracking-tight">
            수산물 도매
            <span className="block bg-gradient-to-r from-sky-400 via-cyan-300 to-sky-400 bg-clip-text text-transparent">
              이제 쉽게
            </span>
          ...
          <p className="text-slate-400 text-lg md:text-xl mb-12 leading-relaxed max-w-2xl mx-auto">
            주문부터 재고, 세금계산서까지<br className="hidden md:block" />
            수산물 도매에 필요한 모든 것을 한 곳에서
          </p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <Link href="/shop"
              className="bg-sky-500 hover:bg-sky-400 text-white font-bold px-10 py-4 rounded-2xl transition-all duration-200 hover:scale-105 hover:shadow-2xl hover:shadow-sky-500/40 active:scale-95">
              🛒 쇼핑몰 바로가기
            </Link>
            <a href="#contact"
              className="bg-white/5 hover:bg-white/10 backdrop-blur border border-white/10 hover:border-white/20 text-white font-semibold px-10 py-4 rounded-2xl transition-all duration-200 hover:scale-105">
              무료 상담 신청
            </a>
          </div>

          {/* 통계 */}
          <div className="flex justify-center gap-6 sm:gap-12 mt-12 sm:mt-20 flex-wrap">
            {[
              { num: '100%', label: '국내 서버' },
              { num: '6가지', label: '핵심 기능' },
              { num: '24/7', label: '언제든 접속' },
              ...
              <div key={s.label} className="text-center">
                <p className="text-4xl font-black bg-gradient-to-r from-sky-400 to-cyan-300 bg-clip-text text-transparent">{s.num}</p>
                <p className="text-slate-500 text-sm mt-1.5 font-medium">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" className="py-16 sm:py-28 px-4 sm:px-6 bg-[#111827]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-sky-400 text-sm font-bold mb-3 tracking-widest uppercase">✨ 핵심 기능</p>
            <h2 className="text-3xl md:text-5xl font-black mb-4">필요한 건 다 있어요</h2>
            <p className="text-slate-400 text-lg">수산물 도매업에 최적화된 6가지 핵심 기능</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <div key={f.title}
                className={`group relative bg-slate-800/60 border border-slate-700/50 rounded-2xl p-7 hover:border-transparent hover:-translate-y-2 hover:shadow-2xl ${f.shadow} transition-all duration-300 overflow-hidden`}>
                {/* 호버시 그라데이션 테두리 효과 */}
                <div className={`absolute inset-0 bg-gradient-to-br ${f.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300 rounded-2xl`} />
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${f.gradient} flex items-center justify-center text-2xl mb-5 shadow-lg ${f.shadow} group-hover:scale-110 transition-transform duration-300`}>
                  {f.icon}
                </div>
      ...
      <section id="cases" className="py-16 sm:py-28 px-4 sm:px-6 bg-[#0a0f1e]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-amber-400 text-sm font-bold mb-3 tracking-widest uppercase">🏆 도입 사례</p>
            <h2 className="text-3xl md:text-5xl font-black mb-4">이미 쓰고 있어요</h2>
            <p className="text-slate-400 text-lg">전남 지역 수산물 업체들이 먼저 경험했습니다</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {cases.map((c, i) => (
              <div key={c.name}
                className={`relative bg-gradient-to-br ${c.color} border ${c.border} rounded-2xl p-7 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 overflow-hidden backdrop-blur`}>
                <div className="absolute top-0 right-0 text-8xl opacity-10 leading-none pt-4 pr-4">{c.emoji}</div>
                <div className="relative z-10">
                  <span className="text-5xl mb-5 block">{c.emoji}</span>
                  <h3 className="font-bold text-xl text-white mb-1">{c.name}</h3>
                  <p className="text-sky-400 text-xs font-semibold mb-4">📍 {c.location}</p>
                  <p className="text-slate-300 text-sm leading-relaxed">{c.desc}</p>
                </div>
              </div>
            ))}
      ...
      <section id="contact" className="py-16 sm:py-28 px-4 sm:px-6 bg-[#111827]">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-emerald-400 text-sm font-bold mb-3 tracking-widest uppercase">📞 무료 상담</p>
            <h2 className="text-3xl md:text-5xl font-black mb-4">지금 바로 시작하세요</h2>
            <p className="text-slate-400 text-lg">도입 문의 및 데모 신청을 받고 있어요</p>
          </div>

          <div className="bg-slate-800/60 border border-slate-700/50 rounded-3xl p-8 space-y-4 backdrop-blur">
            {[
              { label: '업체명', placeholder: '예) 영광 굴비가게' },
              { label: '연락처', placeholder: '010-0000-0000' },
              { label: '지역', placeholder: '예) 전남 영광' },
              ...
              <div key={f.label}>
                <label className="block text-xs font-bold text-slate-400 mb-1.5 tracking-wide uppercase">{f.label}</label>
                <input type="text" placeholder={f.placeholder}
                  className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl px-4 py-3.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all" />
              </div>
            ))}
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5 tracking-wide uppercase">문의 내용</label>
              <textarea rows={4} placeholder="궁금한 점이나 요청사항을 입력해주세요"
                className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl px-4 py-3.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all resize-none" />
            </div>
            <button className="w-full bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg shadow-sky-500/20 hover:shadow-sky-500/40 text-base">
              상담 신청하기 🐟
            </button>
            <p className="text-center text-slate-500 text-xs">빠른 시일 내에 연락드릴게요</p>
          </div>
      ...
      {/* 푸터 */}
      <footer className="py-10 px-6 border-t border-slate-800 bg-[#0a0f1e]">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xl">🐟</span>
            <span className="font-bold text-slate-400">굴비가게 도매 관리 시스템</span>
          </div>
