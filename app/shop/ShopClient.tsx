...
type ShopClientProps = {
  ...
  initialBanners: any[]
  initialReviewStats: Record<string, { sum: number; count: number }>
}

export default function ShopClient({ initialProducts, initialCategories, initialBanners, initialReviewStats }: ShopClientProps) {
  const [products, setProducts] = useState<Product[]>(initialProducts)
  const [categories] = useState<Category[]>(initialCategories)
  const [reviewStats] = useState<Record<string, { sum: number; count: number }>>(initialReviewStats)
  const [sortBy, setSortBy] = useState<'추천순' | '평점순' | '최신순'>('추천순')
  const [banners] = useState<any[]>(initialBanners)
  const [bannersLoading] = useState(false)
    ...
    <div style={{ background: bg, color: text, minHeight: '100vh', fontFamily: "'Pretendard','Apple SD Gothic Neo',sans-serif", overflowX: 'hidden' }}>
      ...
      {user && memberType !== '일반' && (
        <div style={{
          ...
          {memberType === '도매업' ? '🏭 도매업 회원 — 특별 도매 공급가 적용 중이에요!' : '🏪 소매업 회원 — 소매 공급가 적용 중이에요!'}
        </div>
      )}
