...
export default function SupplierDashboardPage() {
  ...
  return (
    <SupplierLayout>
      <div style={{ minHeight: '100vh', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: '20px' }} className="dash-wrap">

        {/* 헤더 */}
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
          <div style={{ minWidth: 0 }}>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', marginBottom: '4px' }}>{greeting}</p>
            <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'white', margin: '0 0 8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} className="dash-title">{supplier.company_name}</h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '11px', fontWeight: 700, padding: '4px 12px', borderRadius: '20px', background: statusBg[supplier.status] || 'rgba(255,255,255,0.1)', color: statusColor[supplier.status] || 'white', border: `1px solid ${statusColor[supplier.status] || 'transparent'}30` }}>
                {supplier.status === '승인' ? '✅ 승인된 공급업체' : supplier.status === '대기중' ? '⏳ 승인 심사중' : '❌ 거절됨'}
              </span>
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '11px' }}>{new Date(supplier.created_at).toLocaleDateString('ko-KR')} 가입</span>
            </div>
          </div>
          {/* 시계 — 태블릿 이상만 */}
          <div style={{ textAlign: 'right', flexShrink: 0 }} className="dash-clock">
            <p style={{ fontSize: '28px', fontWeight: 800, color: 'white', margin: 0, fontVariantNumeric: 'tabular-nums', letterSpacing: '-1px' }}>
        ...
        {supplier.status === '대기중' && (
          <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '16px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', margin: 0 }}>승인 후 상품 등록이 가능합니다. 영업일 기준 1~2일 소요됩니다.</p>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          {stats.map(s => (
            <div key={s.label} style={{ background: s.bg, border: `1px solid ${s.color}20`, borderRadius: '16px', padding: '18px 16px' }}>
              ...
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', margin: '0 0 4px' }}>{s.label}</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <Link href="/supplier/products" style={{
            ...
            background: supplier.status === '승인' ? 'rgba(245,158,11,0.12)' : 'rgba(255,255,255,0.03)',
            border: `1px solid ${supplier.status === '승인' ? 'rgba(245,158,11,0.25)' : 'rgba(255,255,255,0.06)'}`,
            color: supplier.status === '승인' ? '#f59e0b' : 'rgba(255,255,255,0.3)',
          }}>
            <span style={{ fontSize: '26px' }}>➕</span>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '13px', fontWeight: 700, margin: 0 }}>상품 등록</p>
              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', margin: '3px 0 0' }}>
                {supplier.status === '승인' ? '새 상품 등록' : '승인 후 이용'}
              </p>
            </div>
          </Link>
          <Link href="/supplier/products" style={{
            ...
            <span style={{ fontSize: '26px' }}>📋</span>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '13px', fontWeight: 700, margin: 0 }}>상품 목록</p>
              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', margin: '3px 0 0' }}>등록 상품 관리</p>
            </div>
          </Link>
        </div>

        {/* 최근 등록 상품 */}
        <div style={{ background: '#161b22', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: '14px', fontWeight: 700, color: 'white', margin: 0 }}>최근 등록 상품</h2>
          ...
          {products.length === 0 ? (
            ...
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', flexShrink: 0, overflow: 'hidden', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {p.image_url ? <img src={p.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: '18px' }}>🐟</span>}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'white', margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
              ...
              <span style={{ fontSize: '10px', fontWeight: 600, padding: '3px 9px', borderRadius: '20px', flexShrink: 0, background: statusBg[p.approval_status] || 'rgba(255,255,255,0.05)', color: statusColor[p.approval_status] || 'rgba(255,255,255,0.4)' }}>
        ...
        {/* 업체 정보 */}
        <div style={{ background: '#161b22', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', padding: '20px' }}>
          <h3 style={{ fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', margin: '0 0 14px', letterSpacing: '1px', textTransform: 'uppercase' }}>업체 정보</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[
              ...
              <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 500, color: 'rgba(255,255,255,0.8)', textAlign: 'right', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.value}</span>
              </div>
            ))}
          </div>
        </div>
